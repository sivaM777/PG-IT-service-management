...
async function findLeastLoadedAgent(): Promise<string | null> {
  const res = await pool.query<{ id: string }>(
    `SELECT u.id
     ...
     ORDER BY (COALESCE(w.open_tickets_count, 0) + COALESCE(w.in_progress_tickets_count, 0)) ASC, u.created_at ASC
     LIMIT 1`
  );
...
export async function findMatchingRules(args: {
  ...
  for (const rule of rules.rows) {
    ...
    if (rule.keyword_filter && rule.keyword_filter.length > 0) {
      const allKeywordsPresent = rule.keyword_filter.every((keyword) =>
...
export async function getAgentWorkload(agentId: string): Promise<{
  ...
  const result = await pool.query(
    `SELECT open_tickets_count, in_progress_tickets_count 
     FROM agent_workload 
     WHERE agent_id = $1`,
    [agentId]
  );
  ...
  if (result.rows.length === 0) {
    return { openTickets: 0, inProgressTickets: 0, totalWorkload: 0 };
  }
...
export async function findBestAgentForCategory(
  ...
  let query = `
    SELECT 
      s.agent_id,
      s.skill_level,
      COALESCE(w.open_tickets_count, 0) + COALESCE(w.in_progress_tickets_count, 0) as total_workload
    FROM agent_skills s
  ...
  query += `
    ORDER BY s.skill_level DESC, total_workload ASC
    LIMIT 1
  `;
