...
export async function sendAlert(
  ...
  // Send emails
  if (rule.channels.includes("EMAIL") && recipients.emails.length > 0) {
  ...
  // Send SMS
  if (rule.channels.includes("SMS") && recipients.phones.length > 0) {
  ...
  if (rule.channels.includes("IN_APP") && recipients.userIds.length > 0) {
    promises.push(
      createInAppNotifications(rule, ticketData, recipients.userIds).catch((err) => {
...
async function createInAppNotifications(
  ...
  await pool.query(
    ...
    [userIds, data.ticketId, data.eventType, title, body]
  );
}
