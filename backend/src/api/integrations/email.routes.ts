...
export const emailRoutes: FastifyPluginAsync = async (server) => {
  ...
  server.get(
    ...
    async (request, reply) => {
      const result = await pool.query(
        "SELECT id, name, email_address, imap_host, imap_port, imap_secure, enabled, last_checked_at, created_at FROM email_sources ORDER BY created_at DESC"
      );
  ...
  server.post(
    ...
    async (request, reply) => {
      ...
      try {
        await emailMonitor.checkSource(params.id);
        return reply.send({ message: "Email check completed" });
      } catch (err: any) {
